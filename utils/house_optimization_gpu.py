"""
HOUSE-ONLY FAST GPU TUNING — XGBoost + CatBoost
...
VAL_SIZE = 0.15           # validation share from train_val
RANDOM_STATE = 42
N_TRIALS = 100
...
# =========================================================
# FEATURE ENGINEERING HELPERS (AS PROVIDED)
# =========================================================
...
def transform_stage3(df, fitted):
    ...
    # backup geo columns BEFORE imputation
    ...
    # restore raw geo after imputation
    ...
    # final imputation (including TE + log)
    df = impute_features(df, fitted["numeric_medians"], fitted["ordinal_modes"])
...
# =========================================================
# OPTUNA OBJECTIVES — MINIMIZE MAE (R² FOR REPORTING ONLY)
# =========================================================
...
# =========================================================
# MAIN EXECUTION — HOUSES ONLY
# =========================================================

def main():
    log_lines = []
    log_lines.append("HOUSE-ONLY FAST GPU TUNING — XGB + CAT")
    log_lines.append("=" * 70)

    print("=" * 70)
    print("HOUSE-ONLY FAST GPU TUNING — XGBoost + CatBoost")
    print("=" * 70)

    # -----------------------------
    # Load data & filter to houses
    # -----------------------------
    df_full = pd.read_csv(DATA_PATH)
    df = df_full[df_full["property_type"] == "House"].copy()

    log_lines.append(f"Total rows (all): {len(df_full)}")
    log_lines.append(f"Rows after filtering property_type == 'House': {len(df)}")

    df = df.drop(columns=["url", "address"], errors="ignore")

    # -----------------------------
    # Split BEFORE Stage 3
    # -----------------------------
    ...
    # -----------------------------
    # OPTUNA XGBOOST (minimize MAE)
    # -----------------------------
    print("\n=== OPTUNA: XGBoost (HOUSE) — objective = Val MAE ===")
    log_lines.append("\n=== OPTUNA: XGBoost (HOUSE) — objective = Val MAE ===")
    ...
    # Train final XGB with best params on train only (keeping val for metrics & ensemble)
    ...
    # -----------------------------
    # OPTUNA CATBOOST (minimize MAE)
    # -----------------------------
    print("\n=== OPTUNA: CatBoost (HOUSE) — objective = Val MAE ===")
    log_lines.append("\n=== OPTUNA: CatBoost (HOUSE) — objective = Val MAE ===")
    ...
    # -----------------------------
    # ENSEMBLE SEARCH (MAE-BASED)
    # -----------------------------
    print("\n=== WEIGHTED ENSEMBLE SEARCH (HOUSE, objective = Val MAE) ===")
    log_lines.append("\n=== WEIGHTED ENSEMBLE SEARCH (HOUSE, objective = Val MAE) ===")
    ...
    # WRITE SUMMARY TXT
    # -----------------------------
    log_path = Path(OUTPUT_DIR) / "house_tuning_summary.txt"
